...
// SlidingWindow tracks a rate over a 60-second sliding window using per-second buckets.
...
// RawBuckets returns a copy of the per-second bucket values and the unix second of the
// newest bucket. Callers can reconstruct timestamps as lastSec - (windowBuckets-1-i) for index i.
func (w *SlidingWindow) RawBuckets() (buckets [windowBuckets]int64, lastSec int64) {
	w.mu.Lock()
	defer w.mu.Unlock()
	now := time.Now().Unix()
	w.advance(now)
	return w.buckets, w.lastSec
}
